...
**Git-connecting the two Vercel projects: half worked.** The goal was push-to-deploy instead of a manual `vercel deploy` per change. Vercel supports multiple projects from one repo, distinguished by each project's **Root Directory**, which is the setting whose absence caused the earlier API-deployed-over-the-frontend incident. `division-api` connects cleanly: its Root Directory is the repo root, which is exactly where its `vercel.json` and `requirements.txt` live, so a git-triggered build produces the same correct Python function as a CLI deploy (verified: a push auto-deployed it and `/api/stats` stayed 200). It is now git-connected and deploys on push.

`division` could not be. It needs Root Directory `web` -- at the repo-root default a git build reads the root `vercel.json`, the API's config, and redeploys the API over the frontend. But with Root Directory set to `web`, the git-triggered build compiled Next.js successfully (build log shows all seven routes generated) and yet Vercel registered **zero build outputs**: `vercel inspect --json` reports `output count: 0`, against 29 for a working CLI deploy. The deployment therefore served only raw `public/` files at the root -- every page 404'd, including through `nicklam.co`, and notably the un-prefixed `/maplibre/maplibre-gl-worker.mjs` returned 200 while every `/division/*` path did not, which is what identified it as an output-collection failure rather than a routing or `basePath` problem. Ruled out along the way: the root `vercel.json` leaking in (the build record shows `vercelConfig: {}`), and deployment protection (`division-api` carries the identical `all_except_custom_domains` setting and works). Reverted `division` to Root Directory `.` with git disconnected and redeployed from `web/` via CLI, which restored production. Unresolved -- worth retrying via the dashboard, which may configure this differently than the CLI, and `next.config.ts`'s `turbopack.root` is a plausible suspect to test.
