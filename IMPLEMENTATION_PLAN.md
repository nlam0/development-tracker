...
- **`division-api`** (`api/`) → `https://division-api-one.vercel.app`. Deployed from the *repo root* (not `api/`) via the root-level `vercel.json` + `requirements.txt`, so `api/`'s absolute imports resolve unchanged.
