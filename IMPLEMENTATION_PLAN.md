...
**Status:** M0-M7 complete (V1's full milestone sequence, PRD §16), plus a post-M3 data-integrity audit (findings folded into R1 and R12; decisions D6 and D7 resolved and implemented). The study area resolves to **1,954 parcels** (Chinatown 501, Two Bridges 523, Lower East Side 930), all loaded into `parcels`. M4 has loaded **10,364 study-area DOB NOW permits** into `permits` (10,085 by BBL allowlist, 279 by D7(b) spatial match), verified idempotent. M5 has built all six PRD §11 endpoints (`api/main.py`, `api/routers/`) plus two small additions found necessary while building M6 (a `block` filter, `GET /api/study-areas`); `/api/activity` and `/api/map` share one filter grammar (`api/filters.py`) and return identical record sets for identical params, `/api/activity` is keyset-paginated, and `/api/stats` serves 7/30/90-day digest windows. M6 has built the full frontend (`web/`) -- feed, map, parcel pages, watchlist, methodology -- against the live API, verified via SSR and a full production build (`next build`/`next lint` clean); four real bugs the M6 section flagged as unverified were found and fixed in the user's own browser testing (see "Post-M6 fixes"). M7 has deployed both halves live to Vercel as **Division** -- `division` (web) at `https://division-theta.vercel.app`, `division-api` at `https://division-api-one.vercel.app` -- plus a daily GitHub Actions ingestion cron; see the M7 section for the deployed URLs, the two-Vercel-projects rationale, and the deploy bugs found and fixed. M0 scaffolding and M1's study areas are committed and pushed (`origin/main`); M1's resolved BBL set is 1,944 parcels (Chinatown 501, Two Bridges 522, Lower East Side 921). M2 has applied the full §3 schema to Supabase -- 9 application tables with all constraints and indexes verified. M3 has loaded all 1,944 study-area parcels from PLUTO into `parcels`, verified idempotent. See each milestone section below for details.
...
**Done.** All six PRD §11 endpoints, built in the shape §2's architecture sketch describes: `api/db.py` (pooled connection), `api/filters.py` (shared filter grammar), `api/models.py` (Pydantic response models), `api/routers/{activity,map,parcels,stats}.py`, `api/main.py` (app + router mounting). Verified live against Supabase with a real `uvicorn` process and `TestClient`, not just unit-tested: filters, cursor pagination, sort ordering, 404/422 behavior, and cross-endpoint consistency between `/api/activity` and `/api/map` were all exercised against the actual 10,364-permit / 1,954-parcel dataset. 36 new tests (`tests/test_api_*.py`), all passing; full suite is 98 passing, skips gracefully to 44 (the pure, non-DB tests) with `.env` hidden, `ruff check .` clean.

`api/filters.py`'s `ActivityFilters` (neighborhood, category, source, date range, cost range) is the single filter grammar both `/api/activity` and `/api/map` build their `WHERE` clause from — verified directly (`tests/test_api_map.py::test_map_matches_activity_for_the_same_filters`) rather than just by shared code: for identical `neighborhood`/`category` params, the two endpoints return the exact same permit-id set.

`/api/activity` uses keyset (not offset) pagination, per the milestone's instruction, so pages stay stable as new permits are ingested underneath the feed rather than shifting or duplicating rows the way offset pagination would. Every sort mode (`newest`, `oldest`, `cost`) is keyed on a `(null-rank, value, id)` triple, not just `(value, id)`: `estimated_cost` is nullable, and Postgres's raw tuple comparison against a NULL evaluates to NULL — never true — which would silently drop every null-cost permit from every page after the first under `sort=cost`. The rank component (0 for a present value, 1 for NULL) makes NULLs sort last under both directions and keeps every component of the keyset predicate non-null, so the comparison is always well-defined. A cursor also encodes which sort mode it was issued under and is rejected (422) if reused with a different one, since the two aren't interchangeable positions.

Two things found while building this, beyond the one-liner:

- **pgbouncer transaction-pooler compatibility.** Risk R7 already called for connecting through Supabase's pooler (port 6543) rather than the direct port, but building `api/db.py` surfaced a specific failure mode beyond just "pool connections": psycopg's default behavior promotes a repeated query to a server-side prepared statement after a few executions, and under pgbouncer's transaction-pooling mode a "prepared" logical connection can be hosted on a different backend between requests, so a later execute can fail with "prepared statement does not exist." `api/db.py`'s pool is opened with `prepare_threshold=None` (disables autoprepare) and `autocommit=True` (no request holds a connection idle-in-transaction against a pooler that's meant to keep handing connections back out). Both are documented inline in `api/db.py` rather than left as unexplained kwargs.
- **`ORDER BY 0` is not a literal zero.** The keyset query's rank component was originally a bare `0`/`1` in the `SELECT`-less `ORDER BY` clause (`ORDER BY {rank_expr} ASC, ...`); Postgres parses a bare integer in `ORDER BY` as an ordinal reference to a `SELECT`-list column position, not a literal value, and raised `ORDER BY position 0 is not in select list` the first time this ran against live Supabase. Fixed by casting (`0::int`), which forces it to parse as an expression. Caught by the live smoke test before it ever reached the test suite.

`/api/map` returns every matching point as a GeoJSON `FeatureCollection` up to a 5,000-row hard cap, not a paginated slice — PRD §7B's "marker clustering at low zoom" is MapLibre's own client-side GeoJSON-source clustering, which needs the whole matching set to cluster correctly, so cursor pagination doesn't apply here the way it does to the feed.

`/api/parcels/{bbl}/permits` and `/api/parcels/{bbl}/records` 404 when the parcel itself doesn't exist, rather than returning an empty list for any 10-digit string — verified by test. This is also why a spatially-matched permit (D7(b), `permits.bbl IS NULL`) is unreachable through this router: it has no parcel to be a sub-resource of, consistent with the parcel-page limitation the README already documents. `/api/parcels/{bbl}/records` currently always returns `[]`, since `property_records` has no ACRIS data until M8 — verified this returns `200 []` rather than erroring, not left unverified.

`/api/stats` returns all three digest windows (7/30/90 days) in one response rather than one per request, since PRD §7E's digest UI shows them together. Scoped to `permits` only (`property_records` isn't populated until M8); `properties_with_activity` uses `count(DISTINCT bbl)`, which naturally excludes spatially-matched (`bbl IS NULL`) permits from that particular count — the same limitation as the parcel-page case, not a new one.
...
**Done.** All five routes from §2's architecture sketch (`/`, `/map`, `/parcel/[bbl]`, `/watchlist`, `/methodology`), built against the live M5 API. `web/lib/filters.ts` is the "one filter state object" the milestone calls for -- it lives in the URL query string, not React context or a store, so `/` and `/map` read and write the exact same representation and stay in sync across navigation without any state lifted between them; its param names match `api/filters.py`'s `ActivityFilters` field-for-field so they forward to the API almost unchanged. `next build` and `next lint` both pass clean (`next lint` includes React Compiler's stricter hook-correctness rules, not just style checks -- see below), and every route was exercised against a live `uvicorn` + `next dev` pair: SSR responses for all five routes, a real parcel page rendering actual permit data end-to-end, and a 404 for an unknown BBL. **Not verified**: actual in-browser interactivity (MapLibre marker rendering/clustering/click handling, checkbox-driven URL updates, localStorage watchlist persistence, clipboard copy) -- no browser automation tool is available in this environment, so those paths are verified by code review and SSR/build correctness only, not by driving a real browser. Flagging this explicitly per this project's own "if you can't test the UI, say so" standard rather than claiming full verification.

Two gaps found while building this, both small additions to the M5 API rather than to M6 itself, following the same audit discipline as D6/D7 and the M4 category fix:

- **No way to query permits by block.** PRD §7D's watchlist supports bookmarking a block, but M5 shipped no block filter -- there was no PRD-listed feature that needed one yet. Added `block` (5-digit BBL block, chars 2-6) to `api/filters.py`'s `ActivityFilters`, which both `/api/activity` and `/api/map` already share, so it required no per-endpoint work. Reuses the exact `substring(bbl from 2 for 5)` expression `api/routers/stats.py` already used for `blocks_with_multiple_filings`.
- **No endpoint served `study_areas` geometry.** PRD §7B requires the study-area boundary to be visible on the map, and CLAUDE.md is explicit that boundaries "must be stored explicitly in the application (not hardcoded per-query)" -- so baking a copy of the boundary polygon into `web/` would have violated that instruction directly. Added `GET /api/study-areas`, a read-only `ST_AsGeoJSON` view over the same `study_areas.geom` M1 already treats as the single source of truth everywhere else. Both additions are covered by new tests in `tests/test_api_map.py`; full backend suite is 102 passing.

Design decisions made while building, recorded here rather than left implicit in the code:

- **Watchlist scope.** PRD §7D asks for three bookmark types -- parcel, address, block -- but the API can only resolve two of them into a real activity feed. A "parcel" bookmark uses `GET /api/parcels/{bbl}/permits` directly; a "block" bookmark uses the new `block` filter above. An "address" bookmark has no server-side match: R2 (§7 Risks) already decided against building fuzzy address-to-BBL matching during M4, on the reasoning that D7(b)'s spatial fallback covers what it would have caught anyway. `web/lib/watchlist.ts` still lets a researcher save a free-text address as a note, but the watchlist page says plainly that it has no automatic activity feed, rather than silently showing nothing and letting the researcher assume it's being tracked.
- **Basemap.** CLAUDE.md confirms MapLibre GL JS but not a tile provider or API key. `web/components/MapView.tsx` uses MapLibre's own public demo style (`demotiles.maplibre.org`, OSM-derived, no key) as an explicit placeholder -- swapping `MAP_STYLE_URL` is the only change needed once a real basemap/key decision is made, likely alongside deploy (M7).
- **Research digest has no dedicated route.** PRD §7E describes the digest as part of core UX, but §2's `app/` sketch names only five routes and doesn't include one for it. Built as a panel (`DigestPanel.tsx`) on the feed page -- the landing page -- rather than inventing a sixth route the plan never called for.

**Post-M6 fixes.** The "not in-browser verified" gap above was exactly where the real bugs turned out to be -- all four were reported from the user's own browser session, not caught by `next build`/`next lint`/SSR:

- `useWatchlist()` crashed with "Maximum update depth exceeded." `getWatchlist()` built a fresh, freshly-sorted array on every call, so `useSyncExternalStore`'s `getSnapshot` returned a new object reference on every render even when nothing had changed -- React reads that as "still changing" forever. Fixed by caching the snapshot and invalidating it only inside `notify()`, i.e. only on an actual write.
- The parcel page's two watch buttons ("parcel" and "block") rendered identical "Watch"/"Watching" text, so watching a parcel vs. the block it sits on -- both deliberate PRD §7D bookmark types, not a duplicate control -- was indistinguishable. `WatchlistButton` gained an `actionLabel` prop ("this parcel" / "block 00162") so a page showing more than one reads as distinct actions.
- The map rendered as a flat blank blue screen. MapLibre's public demo style (`demotiles.maplibre.org`) is a world map of country outlines for demoing the library, not a real basemap -- no tile data below country zoom, so at city-block zoom all that ever painted was its own flat `#D8F2FF` background layer. Swapped to OpenFreeMap's "Positron" style (`tiles.openfreemap.org`) -- free, no signup, no API key, real OSM street/building vector tiles -- which also fits the restrained visual direction better. Still a default pending a deliberate basemap/provider decision, not a final pick.
- With the real basemap in place, the map still showed only its background color (now gray, Positron's background, not blue) -- gray, not blank, but still no roads, buildings, or land. Browser devtools showed: `Failed to load module script: The server responded with a non-JavaScript MIME type of "text/html"`. Root cause: maplibre-gl v6 decodes vector tiles in a dedicated module Web Worker, created as `new Worker(url, {type: "module"})` with a URL the library computes via a bundler-relative import at runtime. Turbopack doesn't resolve that correctly for this package -- the request fell through to Next's own page router and came back as the page's HTML instead of JS, which the browser's strict module-script MIME check then refused to execute. The background layer needs no worker (a flat paint color), but every data-driven layer does, so none of them ever rendered -- exactly the symptom. Fixed by serving maplibre-gl's own prebuilt worker bundle as a plain static asset (`web/public/maplibre/`, generated fresh on `npm install` by `web/scripts/copy-maplibre-worker.mjs` rather than committed, so it can't drift from the installed `maplibre-gl` version) and pointing `maplibregl.setWorkerUrl()` at it directly, bypassing the bundler's URL resolution entirely. Verified via `curl` against the running dev server that both files now serve with `Content-Type: application/javascript`.
- The parcel page's census tract 2020 looked wrong next to 2010 -- e.g. 1 East Broadway showed `1002700` (2020) beside `27` (2010), which reads as nonsense side by side. Verified against the live DB and PLUTO's raw Socrata data directly that this is a display bug, not a data one: `pipeline/sources/pluto.py` passes both fields through unchanged, and PLUTO itself just formats the two vintages differently -- `bct2020` packs borough(1) + tract-whole(4, zero-padded) + split-suffix(2) with no separators (`"1002700"` = borough 1, tract 27.00), while `ct2010` is already ingested in the plain `"N"`/`"N.NN"` form. Decoded, `"1002700"` *is* `"27"` -- the same tract. Nearby parcels (e.g. 34 Mott Street) show a real tract split between 2010 and 2020 (`"29"` -> `"29.02"`), exactly the boundary-vintage risk R11 already flags, and the fix deliberately still shows that rather than masking it. `web/lib/format.ts` gained `formatCensusTract2020()` to decode `bct2020` for display only -- storage stays raw, since the exact string is what the eventual ACS join (M8) needs to match against.
...
**Done.** The project is deployed as **Division** -- named for Division Street, the D1 hand-split line -- intended at `nicklam.co/division`. `.github/workflows/ingest.yml` runs `pluto` then `dob_now` daily via GitHub Actions; both adapters already returned 1 on failure before this milestone (`pipeline/run.py`'s `sys.exit(main())`), so no pipeline code changed, only the schedule and secrets.

Deployed live via the Vercel CLI as two separate projects, not Vercel's single-project Next.js-plus-Python-functions convention -- see §2's "Backend hosting decision" for why the repo keeps `api/` and `web/` as siblings, and the deviation note below for the specific import-path conflict that ruled the single-project convention out:

- **`division`** (`web/`) → `https://division-theta.vercel.app` (aliased; a real custom domain wasn't set up this session). `next.config.ts`'s `basePath: "/division"` makes every route resolve under `/division`, verified live: `/division`, `/division/map`, `/division/watchlist`, `/division/methodology` all 200, root `/` 404s as expected.
- **`division-api`** (`api/`) → `https://division-api-one.vercel.app`. Deployed from the *repo root* (not `api/`) via the root-level `vercel.json` + `requirements.txt`, so `api/`'s absolute imports resolve unchanged. In practice Vercel's Python builder installed from `pyproject.toml` (found at the same root) rather than the scoped `requirements.txt` -- functionally fine (confirmed live), but it means `pipeline/`'s dependencies (`requests`, `shapely`, `uvicorn`, `pydantic-settings`) get installed into the function too, not just the four packages `api/` actually imports. A minor cold-start/bundle-size inefficiency, not a correctness issue -- worth a closer look if cold starts become noticeable, not urgent now.

Verified end-to-end against the live deployments, not just that each one individually returns 200: `/api/activity`, `/api/map`, `/api/stats`, `/api/study-areas` all return real study-area data from `division-api`; `division`'s parcel page (`/division/parcel/1002790065`) renders real permit data server-side; `CORS_ALLOWED_ORIGINS` on `division-api` was confirmed to reflect `access-control-allow-origin` for `division`'s real origin and return nothing for an untrusted origin; and `division`'s compiled client JS bundle was inspected directly to confirm `NEXT_PUBLIC_API_URL` was actually baked in at build time (a Next.js `NEXT_PUBLIC_*` var has to be present *at build time*, not just at runtime, since it's inlined into the client bundle rather than read from the server environment per-request).

`nicklam.co` itself is a separate, already-live Vercel project (`my-site`) outside this repo, updated within hours of this session by other means -- not touched here. Making `nicklam.co/division` actually work needs a rewrite added there once, which the user will add themselves (see README's Deployment section for the exact rule) rather than this session touching a repo it doesn't have.

Two things found while deploying, beyond the milestone's one-liner:

- **Two Vercel projects, not one.** Vercel's documented convention for a Next.js app plus Python serverless functions in one project expects the Python `api/` directory to sit *inside* the Next.js app's own root (`web/api/*.py`). This repo's `api/` is a sibling of `web/`, and its modules use absolute imports (`from api.db import ...`, `from api.routers import ...`) that depend on the *repo root* being on the import path -- exactly the structure CLAUDE.md's architecture calls for ("two independent halves ... must not become coupled"). Restructuring the repo to fit Vercel's convention, or rewriting every `api/` import to relative, would have been deploy-tooling-driven churn on already-tested code for no functional benefit -- so `division` and `division-api` are two separate Vercel projects instead, `division-api` deployed from the repo root specifically so `api/`'s imports need zero changes.
- **`vercel env add` piped a trailing newline into a secret.** The first `division-api` deploy 500'd on every request: `FATAL: database "postgres " does not exist` -- note the trailing space inside the quoted database name. `grep '...' .env | cut -d'=' -f2- | vercel env add ...` preserved the input's trailing newline in the stored value, which became part of the parsed database name. Fixed by using `printf '%s' "$(...)"` instead of a raw pipe -- command substitution `$(...)` strips all trailing newlines unconditionally, guaranteeing a clean value regardless of the source file's line ending. Re-verified with `vercel logs` and a live `/api/stats` request after the fix.

**Post-M7 follow-up.** Once the user added the `nicklam.co/division` rewrite to `my-site` (outside this repo, per the M7 section above), `division-api`'s `CORS_ALLOWED_ORIGINS` needed the real public domain, not just the Vercel preview origin. `nicklam.co` (no `www`) 308-redirects to `www.nicklam.co` -- verified directly with `curl -I` -- so a browser landing on `nicklam.co/division` actually makes its client-side API calls from the `www.nicklam.co` origin; both are allowed regardless, since the redirect behavior isn't guaranteed to stay that way. Updated (again via `printf '%s' "..." | vercel env add ...`, per the trailing-newline lesson from the first deploy) to `http://localhost:3000,https://division-theta.vercel.app,https://nicklam.co,https://www.nicklam.co` and redeployed; verified live that all three real origins get a matching `access-control-allow-origin` and an untrusted origin gets none.

**`division`'s production deployment silently got replaced by the wrong build.** When `division` (`web/`) was first linked (`vercel link --yes --project division`, run from inside `web/`), the CLI's `--yes` default also connected the project's GitHub integration -- printed as "Connecting GitHub repository ... Connected" at the time, easy to read as routine rather than as a consequential default. `division-api`'s later manual `vercel link` didn't print that, so only `division` ended up git-connected. Git-triggered deploys always build from the *repository root*, not from wherever a manual `vercel deploy` happened to be run -- so the next `git push` (the CORS-documentation commit, which touched no application code at all) silently rebuilt and redeployed `division`'s production URL using the *repo-root* `vercel.json`, which defines the **Python API** build (`api/main.py`), not the Next.js app. The user's report -- "no deployment for division" in the dashboard -- was this: `https://division-theta.vercel.app/` was serving FastAPI's `{"detail":"Not Found"}` JSON instead of the Next.js app, confirmed with a plain `curl`.

Fixed with `vercel git disconnect` on the `division` project, then a fresh `vercel deploy --prod --yes` run from inside `web/` to restore the correct build. `division-api` was never git-connected and was unaffected throughout (confirmed live before and after). Deliberately did **not** re-enable git integration for either project afterward -- both now deploy exclusively via explicit CLI runs from the correct directory, which is what actually determines the right `vercel.json`/build for each, rather than relying on a project's git-integration "Root Directory" setting (not fixable from this CLI version) to get it right automatically. Verified live afterward: `/division`, `/division/map`, `/division/watchlist`, `/division/methodology` all 200 with real Next.js content (checked the page `<title>`, not just status codes), a parcel page renders real data, and `https://www.nicklam.co/division` resolves end-to-end through the `my-site` rewrite.

**The map broke a third time -- `basePath` and the MapLibre worker URL.** Adding `basePath: "/division"` in M7 moved everything under `public/` to `/division/...`, but Next only auto-applies `basePath` to `next/link` and `next/navigation` -- never to a path written as a plain string. `MapView.tsx`'s `setWorkerUrl("/maplibre/maplibre-gl-worker.mjs")`, added when the worker was first fixed (before `basePath` existed), was exactly such a string, so it kept requesting the pre-`basePath` path. That path 404s to an *HTML page* rather than failing outright, and MapLibre reacts by falling back to fetching the script and wrapping it in a blob worker -- which then hangs on the HTML instead of erroring. The worker therefore never starts, no `.pbf` vector tiles are ever requested, and the map paints its background colour and nothing else, with *neither* a console error nor the on-page error banner firing (the banner only catches errors MapLibre itself reports).

Two things made this one much slower to find than the earlier map bugs, both worth remembering: the symptom was visually identical to the previous (genuinely different) worker bug, and every server-side check passed -- `curl` fetched the worker, the style, the sprite and the tiles instantly, because `curl` doesn't care which path the *application* actually asks for. What finally localised it was the user reporting the same gray screen on **localhost**, which ruled out the deployment entirely, plus the Network tab showing the worker request stuck at `pending` and zero `.pbf` requests. Fixed by making `next.config.ts` the single source of truth -- it defines `basePath` once and exposes it as `env.NEXT_PUBLIC_BASE_PATH`, which `MapView` prefixes onto the worker URL -- rather than hardcoding the prefix at the point of use, which would reintroduce this same bug the next time `basePath` changes. The rest of `web/` was audited for the same mistake: every other path is a `next/link` href, a `next/navigation` push, or an absolute URL built from `NEXT_PUBLIC_API_URL`, none of which are affected.

**Map glyph 404s -- our own layer, not the basemap.** After the worker fix the map drew correctly but the console showed repeated 404s for `/fonts/Open%20Sans%20Regular,Arial%20Unicode%20MS%20Regular/0-255.pbf`, with MapLibre reporting it was "rendering codepoint ... locally instead." The tempting reading -- an OpenFreeMap/style incompatibility, fixable by swapping the `glyphs` host -- was wrong. Positron's own layers only ever use `Noto Sans Regular/Bold/Italic`, and OpenFreeMap serves all three (verified: 200 each). The failing stack is the *style spec's default* `text-font`, `["Open Sans Regular", "Arial Unicode MS Regular"]`, which our `cluster-count` symbol layer in `MapView.tsx` fell back to because it set `text-field` and `text-size` but no `text-font`. Fixed by naming a font the endpoint actually serves (`Noto Sans Regular`, chosen over Bold to avoid an unrequested appearance change) rather than by touching the glyph host or the style's stacks, which were both fine. Our other three layers draw no text and need no font. The font name is only valid for a style whose glyphs provide it, so it has to be revisited alongside `MAP_STYLE_URL` -- noted in the code.

**Git-connecting both Vercel projects (resolved).** The goal was push-to-deploy instead of a manual `vercel deploy` per change. Vercel supports multiple projects from one repo, distinguished by each project's **Root Directory** -- the setting whose absence caused the earlier API-deployed-over-the-frontend incident. `division-api` connected cleanly on the first try: its Root Directory is the repo root, exactly where its `vercel.json` and `requirements.txt` live, so a git build produces the same Python function a CLI deploy does.

`division` took two attempts. Setting Root Directory to `web` and pushing gave a deployment where Next.js compiled successfully -- the build log shows all seven routes generated -- yet Vercel registered **zero build outputs** (`vercel inspect --json` → `output count: 0`, against 29 for a working CLI deploy). It served only raw `public/` files at the root; every page 404'd, including through `nicklam.co`. The diagnostic that localised it was that un-prefixed `/maplibre/maplibre-gl-worker.mjs` returned 200 while every `/division/*` path did not -- an output-collection failure, not routing or `basePath`.

Cause: the repo-root `vercel.json` declares a top-level `builds` array for the Python function, which puts Vercel into **legacy builds mode** and disables zero-config framework builds. With Root Directory `web` and no `vercel.json` of its own, `division` inherited that root config -- `src: "api/main.py"` matched nothing under `web/`, so nothing was collected, and the config's catch-all route pointed every request at a file the build didn't contain. (This was dismissed too early on the first pass because the build record showed `vercelConfig: {}`; that field reflects what reached the builder, not the deployment-level config.) Fixed by adding `web/vercel.json` (`{"framework": "nextjs"}`), which takes precedence at that Root Directory and carries no `builds` entry. Confirmed by the same measurement that exposed the failure: the next git build reported 29 outputs and `vercelConfig: {"framework": "nextjs"}`, proving it read `web/`'s config rather than the root's. **Two settings must stay in sync: `division`'s Root Directory (`web`) and the existence of `web/vercel.json`.** **Reverted, by decision.** Both projects were then disconnected from GitHub and returned to CLI-only deploys: `division`'s Root Directory back to auto-detect and `web/vercel.json` deleted, since it existed solely to stop git builds inheriting the root config (a CLI deploy from `web/` never reads it -- the build record shows the CLI's own `vercelConfig: {"name": "division", "version": 2}`). The reasoning: push-to-deploy demonstrably worked, but it cost two coupled settings that fail silently and invisibly when they drift apart, which is a poor trade on a single-maintainer project that deploys by hand anyway. The history is kept here in case it's ever revisited. Verified end-to-end afterwards -- one push, both projects built the right artifact (`division` 29 Next.js outputs, `division-api` `λ api/main.py`), and feed, map, watchlist, methodology, a parcel page, the map worker asset and the API all returned 200 through `nicklam.co`.

**"No production deployment" in the dashboard -- a basePath side effect, not a broken deploy.** After the git integration was working, the Vercel dashboard still showed `division` with no production deployment and an error screen, and `https://division-theta.vercel.app/` appeared to be down. Nothing was actually broken: with `basePath: "/division"` the origin root is not a route, so `/` returns 404 while `/division` returns 200. The dashboard's production card previews the deployment **root**, so it rendered that 404 as a failure, and its "Visit" button led to the same place. The API confirmed the project was healthy the whole time (`targets.production` = `READY`/`PROMOTED`, correct `rootDirectory`, correct git link). This was missed twice because every verification checked `/division*` paths and never the bare root -- the one path the dashboard actually shows. Fixed in `next.config.ts` with a root redirect to `basePath`, using `basePath: false` so the redirect source isn't itself prefixed (which would have matched `/division` instead of `/`). Verified the redirect doesn't touch deep paths or the `my-site` rewrite: `/` 307s to `/division`, while `/division/map` still resolves directly with zero redirects.

**Digest counts differed between local and production.** The research digest reported 20 new permits in the past 7 days from a local `uvicorn` and 16 from the deployed API -- against the same Supabase database, so the data was never in question. The boundary was: `api/routers/stats.py` computed the window with `date.today()`, which resolves in whatever timezone the process runs in. Local (Eastern) read 2026-08-31 and started the window at 2026-08-24; the Vercel function (UTC) had already rolled over to 2026-09-01 and started at 2026-08-25. Exactly four permits carried `event_date = 2026-08-24`, which is the whole difference. The same response was also internally inconsistent, stamping `generated_at` in UTC while bounding the windows on a local date.

Fixed by anchoring the windows to the current date in **America/New_York** rather than UTC or server-local. NYC is the correct anchor rather than merely a deterministic one: `permits.event_date` holds NYC calendar dates, so "the past 7 days" has to mean 7 NYC days to mean what a researcher reads it as -- UTC would have been reproducible but still wrong, shifting the window a day early every evening between 20:00 ET and midnight. The boundary is now a small pure function (`window_start`) covered by `tests/test_api_stats_window.py`, which asserts the result is unchanged under four process timezones -- the invariant that was actually violated. Verified after deploying: all three windows (7/30/90) now match direct database counts exactly (20/102/407).

Not yet done, left for a later session: a basemap-tile-provider decision (the map still uses OpenFreeMap's free default, noted in `web/components/MapView.tsx`), and `SUPABASE_DB_URL_POOLED` in `.env` connects on port 5432 (Supabase's *session*-mode pooler) rather than port 6543 (*transaction* mode, what Risk R7 and `api/db.py`'s `prepare_threshold=None` comment were written against) -- it works today (session mode has no prepared-statement caveat to trigger), but is worth the user's attention since it means the app isn't actually running through the pooler mode the code was deliberately designed around.
...
*Mitigation:* connect through Supabase's transaction pooler (port 6543), not the direct port; small per-instance pool sizes; no long-lived transactions in request handlers. The ingestion job — a single long-lived process — was meant to use the literal direct connection instead, but GitHub Actions runners have no IPv6 egress and Supabase's direct-connection hostname (`db.<project-ref>.supabase.co`) resolves IPv6-only without the paid IPv4 add-on, so `.github/workflows/ingest.yml` failed with "Network is unreachable" the first time it ran against a real secret (2026-09-01). Ingestion now connects through Supabase's *session*-mode pooler (Supavisor, port 5432, `aws-0-<region>.pooler.supabase.com`) instead: it's IPv4-reachable like the transaction pooler, but — unlike the transaction pooler — behaves like a real persistent session, so it doesn't hit the prepared-statement caveat `api/db.py`'s `prepare_threshold=None` works around. `SUPABASE_DB_URL_DIRECT` (the env var and secret name) is kept as-is for continuity even though it now holds a pooled connection string, not a literal direct one.
...
**Silent staleness, the other half of R8.** A run can succeed and still leave the database wrong, because both adapters are full reloads that only ever upserted: a record that disappeared upstream — a permit revoked or superseded, a lot merged or demapped out of PLUTO — was never reconciled and sat there indefinitely, indistinguishable from one confirmed by this morning's run. `db/migrations/0013` adds `is_current` to `parcels` and `permits`, and each adapter now marks rows its own completed full scan didn't see (`retrieved_at < run.started_at`), recording the count in `ingestion_runs.records_marked_absent`. Three deliberate properties: it marks rather than deletes, because a permit vanishing from DOB's published set is itself a research finding and deleting it would destroy the only evidence it was filed; the upsert sets `is_current` back to `TRUE`, so a record that reappears recovers; and the sweep is scoped to the source that ran, so a DOB NOW run cannot mark every DOB legacy permit absent once M8 lands. It is safe against a wholesale wipe only because the zero-record guard above already rejects an empty fetch — the two mitigations depend on each other and neither should be removed alone.

`is_current` is surfaced on `PermitOut` rather than filtered out of the feed, map, and digest. Silently dropping records is the failure mode this project keeps finding in itself; a permit that went missing upstream should be visible and labelled, not disappeared a second time. Whether the digest's counts should exclude non-current permits is a research-definition question, not a technical one, and is deferred until there is a single non-current row to reason about.
